...
contains
  ...
  subroutine get_abcissas(N,nuc,r,step)
    ...
    do ii=1,N
      ...
      ! NOTE prefactor 
      x(ii)=(-1.0_dp)*cos(step*dble(ii)) ! gauss-chebyshev abcissas
      r(ii)=(1.0_dp+x(ii))/(1.0_dp-x(ii))*bragg(nuc)
  ...
  subroutine get_abcissas_z_1st(N,nuc,dr,step)
    ...
    do ii=1,N
      ...
      dr(ii)=2.0d0*bragg(nuc)*pi*sin(step*dble(ii))/&
          &(1.0d0+2.0d0*cos(step*dble(ii))+cos(step*dble(ii))**2)
  ...
  subroutine get_abcissas_z_2nd(N,nuc,ddr,step)
    ...
    do ii=1,N
      ...
      ddr(ii)=(-2.0d0*bragg(nuc)*pi**2)*(cos(step*dble(ii))-2.0d0)/&
          &(1.0d0+2.0d0*cos(step*dble(ii))+cos(step*dble(ii))**2)
  ...
  function exp_int(alpha,power,r)
    ...
    do ii=1,power
      exp_int=1.0d0/alpha*r**ii*exp(alpha*r)-dble(ii)/alpha*exp_int
    end do
